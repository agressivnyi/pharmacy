class UsersController < ApplicationController
  ...
  def register_and_send_email
    @user = User.find_by(email: params[:email])
    if @user.nil?
      @user = User.new(email: params[:email], password: SecureRandom.hex(10))  # Assign a random password if none is provided
      if @user.save
  ...
  def update_info_by_user
    decoded_token = JWT.decode(params[:validate_number], Rails.application.credentials.secret_key_base, true, algorithm: 'HS256')
